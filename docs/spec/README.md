...
套用 [醫療與法律事務所情境](../use-cases.md) 時發現的缺口，經第三次盤問確認：

| # | 缺口 | 決定 | ADR |
|---|---|---|---|
| G1 | 導航即成為可信網域，prompt injection 可繞過確認 | 可信網域只來自使用者輸入、使用者核准與全域信任清單；Operator 導航不算 | 0015 |
| G1b | 常用網域每次都要確認 | 提供全域信任網域清單，只能由使用者在 UI 維護，也可在確認卡片勾選加入 | 0010、0015 |
| G2 | 腳本需要以敏感值為參數 | 雲端撰寫的腳本不得帶敏感參數，改由本地處理 | 0017 |
| G2b | 本地處理的方式 | 本地 compute：LLM Flow 由本地模型寫腳本；SLM Flow 用內建確定性資料工具；同一沙箱，輸出不外送 | 0005、0017 |
| G3 | 大量非結構化文字被誤判為 dataset | dataset 只涵蓋表格／紀錄格式；文件類走 R1 分批，超過門檻先確認 | 0005、0017 |
| G4 | 同一案件跨 Thread 別名重編 | 可選的案件 scope，預設仍以 Thread 為 scope | 0009、0015 |
| G5 | 機構預設設定 | 可匯入匯出的隱私設定包，內建醫療、法律兩包；只能變嚴，放寬項目需逐項勾選；v1 不做管理員鎖定 | 0016 |
| G6 | 小格抑制可被略過 | 未宣告或不符 `outputSpec` 的輸出一律只給使用者 | 0017 |
...
6. **預設允許還原**：只擋送往非可信網域的外送，可信網域內或本機動作仍可能被 prompt injection 濫用，靠 Ledger 追查（0015）。全域信任清單越長，這個風險越大。
...
10. **案件 scope**：同一案件內雲端可跨任務拼湊資訊，是使用者以便利換取的已知取捨（0015）。
11. **本地 compute 的品質受本地模型限制**：SLM 模式只能用內建資料工具，任務超出工具能力時只能改用 LLM 模式或人工處理（0017）。
