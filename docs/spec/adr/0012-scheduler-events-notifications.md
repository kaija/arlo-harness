...
3. **觸發流程**：到點 → 建 `schedule_runs` 記錄 → 模板變數替換 → 以 A2A `message/send` 送給指定 Persona，`contextId = schedule:<scheduleId>:<runId>`（每次新 Thread）或 `schedule:<scheduleId>`（延續同 Thread，可設）→ 結果狀態回寫 `schedule_runs`。
...
8. persona.yaml `triggers.events` 可訂閱事件並指定 prompt 模板，模板可用 `{{event.type}}`、`{{event.payload.<path>}}` 及排程變數。訂閱 `task.*` 時可加 `filter: { personaId }` 限制來源，避免自我觸發循環；系統硬性規定同一事件鏈深度上限 5。
9. **本機 HTTP webhook 接收器**：main 開 `127.0.0.1:<port>`（port 可設，預設隨機並顯示於設定頁），路徑 `POST /hooks/<personaId>`，需 `Authorization: Bearer <token>`（token 於設定頁產生、以 safeStorage 加密存放）。body 為 JSON，轉為 `webhook.received` 事件，payload 可在模板中引用。
...
13. Agent 端 API：內建工具 `notify(title, body, severity)` 讓 Persona / Orchestrator 主動推播；系統事件（Task 終態、崩潰、HITL）自動產生通知。
...
[ADR-0005](0005-orchestrator-task-dispatch.md) 派發、[ADR-0009](0009-conversation-threads-and-concurrency.md) Thread、[ADR-0010](0010-hitl-and-risk-levels.md) HITL。
